...
class Classifier(object):
    ...
    def a_classify_euklides(self, test_value: StructNeighbour, kmin: int, kmax: int):
        knn = kNN(kNN.distance_euklides)
        return knn.assign_array(self.neighbourhood, test_value, kmin, kmax)

    def a_classify_chebyshev(self, test_value: StructNeighbour, kmin: int, kmax: int):
        knn = kNN(kNN.distance_chebyshev)
        return knn.assign_array(self.neighbourhood, test_value, kmin, kmax)

    def a_classify_street(self, test_value: StructNeighbour, kmin: int, kmax: int):
        knn = kNN(kNN.distance_street)
        return knn.assign_array(self.neighbourhood, test_value, kmin, kmax)
